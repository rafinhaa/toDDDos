...
describe("Fetch tasks use case", () => {
  beforeEach(() => {
    ...
    sut = new FetchTasksUseCase(inMemoryTasksRepository, inMemoryUsersRepository);
  });

  it("should be to able to fetch tasks", async () => {
    const newUser = makeUser();
    await inMemoryUsersRepository.create(newUser);
    const tasks = Array.from({length: 3}, () => makeTask({ userId: newUser.id }));
  ...
  it("should not be able to fetch tasks from another user", async () => {
    const newUser = makeUser();
    await inMemoryUsersRepository.create(newUser);
    const tasks = Array.from({length: 2}, () => makeTask({ userId: newUser.id }));

    const anotherUser = makeUser();
    await inMemoryUsersRepository.create(anotherUser);
    const tasksFromAnotherUser = Array.from({length: 5}, () => makeTask({ userId: anotherUser.id }));

    const allTasks = [...tasks, ...tasksFromAnotherUser];
    
    await Promise.all(allTasks.map(task => inMemoryTasksRepository.create(task)));
